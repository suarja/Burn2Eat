...
export function validateFoodData(food: FoodData): FoodDataValidation {
  ...
  if (food.imageUrl.includes('unsplash.com') && !food.imageUrl.match(/images\.unsplash\.com\/photo-/)) {
  ...
  if (!validUnits.includes(food.portionSize.unit)) {
    errors.push(`Invalid portion unit: ${food.portionSize.unit}. Must be one of: ${validUnits.join(', ')}`);
  }
