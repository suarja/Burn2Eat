...
export function validateActivityData(activity: ActivityData): ActivityDataValidation {
  ...
  if (!validCategories.includes(activity.category)) {
    errors.push(`Invalid category: ${activity.category}. Must be one of: ${validCategories.join(', ')}`);
  }

  // Intensity validation
  const validIntensities = ['light', 'moderate', 'vigorous'];
  if (!validIntensities.includes(activity.intensity)) {
    errors.push(`Invalid intensity: ${activity.intensity}. Must be one of: ${validIntensities.join(', ')}`);
  }
...
export function generateActivitiesDatasetReport(): string {
  ...
  categories.forEach(category => {
    ...
    const avgMET = (ACTIVITIES_DATASET.filter(a => a.category === category)
      .reduce((sum, a) => sum + a.met, 0) / count).toFixed(1);
  ...
  intensities.forEach(intensity => {
    ...
    const avgMET = (ACTIVITIES_DATASET.filter(a => a.intensity === intensity)
      .reduce((sum, a) => sum + a.met, 0) / count).toFixed(1);
...
export function validateNamingConventions(): {
  ...
  ACTIVITIES_DATASET.forEach(activity => {
    ...
    if (parts.length >= 2 && !commonTypes.some(type => parts[0].includes(type) || type.includes(parts[0]))) {
