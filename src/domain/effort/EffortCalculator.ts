...
export class EffortCalculator {
  ...
  calculateEffort(request: EffortRequest): EffortBreakdown {
    ...
    const primaryEffortItem = this.calculateEffortForActivity(primaryActivity, calories, userWeight)
  ...
  private findAlternativeActivities(request: EffortRequest, primaryActivity: Activity): Activity[] {
    ...
    const candidates = allActivities.filter(
      (activity) => activity.getKey() !== primaryActivity.getKey(),
    )
    ...
    const preferredKeys = request.getPreferredActivityKeys()
    const otherPreferred = candidates.filter(
      (activity) =>
        preferredKeys.includes(activity.getKey()) && activity.getKey() !== primaryActivity.getKey(),
    )

    // Strategy 3: Add popular/default activities if we need more options
    const defaultActivities = this.activityCatalog
      .listDefaults()
      .filter(
        (activity) =>
  ...
  private calculateEffortForActivity(
    ...
    const minutes = this.effortPolicy.minutesToBurn(
      calories as Kilocalories,
      userWeight as Kilograms,
      activityMET,
    )

    return EffortItem.of(activity.getKey(), activity.getLabel(), minutes, activityMET)
  }
  ...
  getComparativeBreakdown(request: EffortRequest): {
    ...
    return {
      light:
        lightActivities.length > 0
          ? this.calculateEffortForActivity(lightActivities[0], calories, userWeight)
          : null,
      moderate:
        moderateActivities.length > 0
          ? this.calculateEffortForActivity(moderateActivities[0], calories, userWeight)
          : null,
      vigorous:
        vigorousActivities.length > 0
          ? this.calculateEffortForActivity(vigorousActivities[0], calories, userWeight)
          : null,
    }
  }
}
