...
export class EffortItem {
  ...
  equals(other: EffortItem): boolean {
    return this.activityKey === other.activityKey && this.minutes === other.minutes
  }
}
...
export class EffortBreakdown {
  ...
  static compose(primary: EffortItem, alternatives: EffortItem[]): EffortBreakdown {
    ...
    const filteredAlternatives = alternatives.filter(
      (alt) => alt.getActivityKey() !== primary.getActivityKey(),
    )
  ...
  getActivitiesInDurationRange(minMinutes: number, maxMinutes: number): EffortItem[] {
    return this.getAllActivities().filter(
      (activity) => activity.getMinutes() >= minMinutes && activity.getMinutes() <= maxMinutes,
    )
  }
    ...
    const combinedAlternatives = [...this.alternatives, ...newAlternatives]
      ...
      .filter(
        (
          alt,
          index,
          array, // Remove alternative duplicates
  ...
  toString(): string {
    ...
    return (
      `EffortBreakdown(Primary: ${this.primary.toString()}, ` +
