...
export class PortionUnitUtils {
  ...
  static fromString(unitString: string): PortionUnit {
    ...
    if (normalized.includes("per 100g") || normalized.includes("pour 100g"))
      return PortionUnit.PER_100G
    ...
    if (normalized === "l" || (normalized.match(/^\d+(\.\d+)?\s*l$/) && !normalized.includes("ml")))
      return PortionUnit.LITERS
  ...
  static getDisplayName(unit: PortionUnit, locale: string = "fr"): string {
    ...
    return (
      displayNames[locale as keyof typeof displayNames]?.[unit] || displayNames.en[unit] || unit
    )
  }
}
