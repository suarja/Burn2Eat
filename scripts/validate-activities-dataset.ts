...
import { 
  ACTIVITIES_DATASET 
} from '../src/infrastructure/data/activities-dataset';
import { 
...
async function main() {
  ...
  const intensityStats = ACTIVITIES_DATASET.reduce((stats, activity) => {
  ...
  metRanges.forEach(range => {
    const count = ACTIVITIES_DATASET.filter(a => 
      a.met >= range.min && a.met < range.max
    ).length;
    const percentage = ((count / ACTIVITIES_DATASET.length) * 100).toFixed(1);
  ...
  const showcaseActivities = [
    'walking_brisk', 'jogging_general', 'running_6mph', 
    'cycling_moderate', 'swimming_moderate', 'crossfit'
  ];
