...
class MockActivityCatalog implements ActivityCatalog {
  ...
  getByIntensity(intensity: 'light' | 'moderate' | 'vigorous'): Activity[] {
    return this.activities.filter(activity => {
      ...
      switch (intensity) {
        case 'light': return met < 3;
        case 'moderate': return met >= 3 && met < 6;
        case 'vigorous': return met >= 6;
  ...
  search(query: string): Activity[] {
    return this.activities.filter(a => 
      a.getLabel().toLowerCase().includes(query.toLowerCase())
    );
  }
}
...
describe('EffortCalculator', () => {
  ...
  describe('calculateEffort', () => {
    ...
    it('should throw error when no activities available', () => {
      ...
      // Act & Assert
      expect(() => emptyCalculator.calculateEffort(request))
        .toThrow('No suitable activity found for effort calculation');
    });
  });
  ...
  describe('calculateEffortWithPolicy', () => {
    it('should calculate effort with different policies', () => {
      ...
      // Assert
      expect(standardResult.getPrimary().getMinutes())
        .toBeLessThan(conservativeResult.getPrimary().getMinutes());
    });
  });
  ...
  describe('Integration with Real MET Formula', () => {
    ...
    it('should show that higher intensity activities require less time', () => {
      ...
      // Assert
      expect(runningBreakdown.getPrimary().getMinutes())
        .toBeLessThan(walkingBreakdown.getPrimary().getMinutes());
    });
  });
