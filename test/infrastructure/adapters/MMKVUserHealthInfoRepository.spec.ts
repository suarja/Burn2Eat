...
describe("MMKVUserHealthInfoRepository", () => {
  ...
  describe("save", () => {
    ...
    it("should throw error when storage save fails", async () => {
      ...
      // Act & Assert
      await expect(repository.save(userProfile)).rejects.toThrow(
        "Failed to save user profile to storage",
      )
    })
  })
  ...
  describe("findById", () => {
    ...
    it("should throw error for invalid data", async () => {
      ...
      await expect(repository.findById(id)).rejects.toThrow(
        "Invalid user profile data found in storage",
      )
    })
  })
  ...
  describe("setCurrent", () => {
    it("should save profile and set as current", async () => {
      ...
      expect(mockSave).toHaveBeenCalledWith("current-user-profile", userProfile.getId().toString())
    })
  })
  ...
  describe("createOrUpdatePrimary", () => {
    it("should create primary profile and set as current", async () => {
      ...
      // Act
      const result = await repository.createOrUpdatePrimary("male" as Sex, 75, 180, ["walking"])
