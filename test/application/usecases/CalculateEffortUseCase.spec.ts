import { CalculateEffortUseCase, CalculateEffortInput } from "../../../src/application/usecases/CalculateEffortUseCase";
...
describe('CalculateEffortUseCase', () => {
  ...
  describe('execute', () => {
    ...
    it('should throw error for missing dish ID', async () => {
      ...
      // Act & Assert
      await expect(useCase.execute(input)).rejects.toThrow('Dish ID and user information are required');
    });
  ...
  describe('getComparativeAnalysis', () => {
    it('should provide comparison across intensity levels', async () => {
      ...
      const hasAnyLevel = result.intensityComparison.light || 
